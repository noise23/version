...
public:
    ...
    // vcoin: kept to ease mechanical porting from json_spirit's Value::get_int()/get_int64()
    int get_int() const { return getInt<int>(); }
    int64_t get_int64() const { return getInt<int64_t>(); }

    enum VType type() const { return getType(); }
    const UniValue& find_value(std::string_view key) const;
};

// vcoin: free-function form kept to ease mechanical porting from json_spirit's
// free-function find_value(const Object&, const std::string&)
inline const UniValue& find_value(const UniValue& obj, std::string_view key)
{
    return obj.find_value(key);
}
